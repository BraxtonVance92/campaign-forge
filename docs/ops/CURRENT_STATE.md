...
- Persistence uses a storage abstraction with three implementations: `B2Storage` (real, production-intended, Backblaze B2 via its S3-compatible API, with paginated object listing so results aren't silently truncated at 1000 keys), `LocalDiskStorage` (an explicitly labeled fallback used automatically when B2 credentials are absent — as they are in this environment), and `InMemoryStorage` (tests only). 59 automated tests pass (`pytest tests/`), and the full flow was additionally exercised manually against a running local server, including the path-traversal, one-source-enforcement, and signature-validation fixes from a review round.
